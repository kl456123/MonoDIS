...
class OverlapsRPNModel(Model):
    def init_param(self, model_config):
        ...
        self.thresh = model_config['thresh']
    ...
    def get_rpn_cls_probs(self, anchors, gt_boxes):
        pass
        """
        anchors: (N,K 4) ndarray of float
        gt_boxes: (N,K 4) ndarray of float
        ...
        gt_boxes_area = (
            ...
            (gt_boxes[:, :, 3] - gt_boxes[:, :, 1] + 1)).view(N, K)

        anchors_area = ((anchors[:, :, 2] - anchors[:, :, 0] + 1) *
                        (anchors[:, :, 3] - anchors[:, :, 1] + 1)).view(N, K)

        # boxes = anchors.view(N, 1, 4).expand(N, K, 4)
        # query_boxes = gt_boxes.view(1, K, 4).expand(N, K, 4)
        boxes = anchors
        query_boxes = gt_boxes
    ...
    def generate_proposal(self, rpn_cls_probs, anchors, rpn_bbox_preds,
        ...
        # fg prob
        fg_criterion = rpn_cls_probs[:, self.num_anchors:, :, :]
        fg_criterion = fg_criterion.permute(0, 2, 3, 1).contiguous().view(
            batch_size, -1)
        # shape(N,K)
        # IoU
        fg_probs = self.get_rpn_cls_probs(proposals,
                                          anchors.expand_as(proposals))
        # fg_probs[fg_criterion < self.thresh] = 0
        fg_probs *= fg_criterion
    ...
    def forward(self, bottom_blobs):
        ...
        proposals_batch, proposals_order, cls_criterion = self.generate_proposal(
    ...
    def loss(self, prediction_dict, feed_dict):
        ...
        cls_criterion = prediction_dict['cls_criterion']
