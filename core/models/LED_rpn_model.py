...
class LEDRPNModel(Model):
    ...
    def iox_pred(self, rpn_conv, rpn_coarse_map_conv, rpn_fine_map_conv):
        ...
        iou_cls = iou_level_probs.view(
            batch_size, 4, self.num_anchors, -1).permute(
                0, 3, 2, 1).contiguous().view(batch_size, -1, 4)
        decoded_iou = self.iox_bbox_coder.decode_batch(iou_cls, iou_reg)

        # used for cls and reg loss
        iou_cls_scores = coarse_map.view(
            batch_size, 4, self.num_anchors, -1).permute(
                0, 3, 2, 1).contiguous().view(batch_size, -1, 4)
    ...
    def forward(self, bottom_blobs):
        ...
        rpn_cls_probs = F.softmax(
            rpn_cls_score_reshape, dim=1).view_as(rpn_cls_scores)
        rpn_cls_probs = rpn_cls_probs.view(
            batch_size, 2, self.num_anchors,
            -1).permute(0, 3, 2, 1).contiguous().view(batch_size, -1, 2)
        rpn_cls_scores = rpn_cls_scores.view(
            batch_size, 2, self.num_anchors,
            -1).permute(0, 3, 2, 1).contiguous().view(batch_size, -1, 2)
    ...
    def loss(self, prediction_dict, feed_dict):
        ...
        iou_cls_loss = self.cls_loss(
            iou_scores.view(-1, 4), iou_scores_targets.view(-1))
        ...
        iog_cls_loss = self.cls_loss(
            iog_scores.view(-1, 4), iog_scores_targets.view(-1))
        ...
        iod_cls_loss = self.cls_loss(
            iod_scores.view(-1, 4), iod_scores_targets.view(-1))
