...
bool eval(string gt_dir, string result_dir, Mail* mail){
    ...
    eval_3d.assign(eval_3d.size(),false);
    ...
    for (int c = 0; c < NUM_CLASS; c++) {
        ...
        if (eval_image[c]) {
            ...
            if(   !eval_class(fp_det, fp_ori, cls, groundtruth, detections, compute_aos, imageBoxOverlap, precision[0], aos[0], EASY, IMAGE,analysis_dir,&indices)
                    || !eval_class(fp_det, fp_ori, cls, groundtruth, detections, compute_aos, imageBoxOverlap, precision[1], aos[1], MODERATE, IMAGE,analysis_dir,&indices)
                    || !eval_class(fp_det, fp_ori, cls, groundtruth, detections, compute_aos, imageBoxOverlap, precision[2], aos[2], HARD, IMAGE,analysis_dir,&indices)) {
